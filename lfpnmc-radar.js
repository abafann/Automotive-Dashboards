...
const DATA_URL = 'https://cdn.jsdelivr.net/gh/abafann/Automotive-Dashboards@main/lfpnmc-radar-data.min.json';

// Tech to highlight with a pulsing halo. Must match a `name` field in the data exactly.
// Set to null to disable.
const HIGHLIGHT_NAME = 'Layered-oxide sodium-ion / hard-carbon cells';

let DATA = null;

const CATEGORIES = [
  { id: 'Battery chemistries', short: 'Chemistries', color: '#4756FF' },
];
...
let activeCategory = 'Battery chemistries';
...
async function init() {
  ...
  try {
    ...
    console.error('LFP-NMC radar: failed to load data from', DATA_URL, err);
    if (status) {
